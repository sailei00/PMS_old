...
//									  +"<td colspan='3' align='left'><span style='color:"+options.titleColor+";font-size:"+options.titleFontSize+";'></td></tr>");			
//		} 
		...
		$.each(dataObj.disks,function (i,n){
			...
			if(imgWidth>0)
			{
				...
				if(i>(options.itelTotal-1))
					...
					itemDiv="<div style='border:1px solid "+ options.itemStyle.borderColor+";background-color:"+options.itemStyle.bgColor+";font-size:"+options.itemStyle.fontSize+"'>";
									var bgcolor;
									if (percentage < options.hazard1) {
										bgcolor = options.itemStyle.normalBgColor;
									}
									if (percentage >= options.hazard1) {
										bgcolor = 'yellow';
									}
									if (percentage >= options.hazard2) {
										bgcolor = options.itemStyle.waringBgColor;
									}
									itemDiv += "<div divWidth='"
											+ imgWidth
											+ "' style='width:0%;height:28px;background-color:"
											+ bgcolor
											+ ";' class='poll_plan" + index
											+ "' >";
									itemDiv += "<div class='plan_e' style='background-color:"
											+ bgcolor
											+ ";'><div class='plan_c'  style='background-color:"
											+ bgcolor
											+ ";'>" +n.money + "元" +
													"</div></div>";
					itemDiv+="</div>";
			...
			//"<tr></tr>"
			var tds="<td width='8%' height='28px' align='right'><strong>"+n.name+"</strong></td><td width='30%' style='bgcolor:"+options.itemStyle.bgColor+";'>"+itemDiv+"</td><td width='12%'>"+"月计划费用："+n.total+"元<br/>(使用率："+percentage+"%)</td>";
			if(i%2==0){
				trs+="<tr>"+tds;
			}else{
				trs+=tds+"</tr>";
